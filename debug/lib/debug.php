...
error_reporting(-1); // E_STRICT not part of E_ALL, use -1 to see all errors
ini_set('display_errors', true);
ini_set('log_errors', 1);
ini_set('error_log', '!phpfatal.log'); // fatal errors
...
function debug_console($display=false)
{
	...
	global $_db; if (!isset($_db)) $_db = false;
	global $Db; if (!isset($Db)) $Db = false;
	static $called = false; if ($called) { return; } else { $called = true; }
	...
	if ($Db) $mysql_time = $Db ? $Db['DebugTime'] : 0;
	else $mysql_time = $_db ? $_db['debug_time'] : 0;
	...
	if ($Db) $count_queries = count($Db['DebugQueries']);
	else if ($_db) $count_queries = count($_db['debug_queries']);
	...
	$ret .= '<div>';
	if (((defined('DB_DEBUG') && DB_DEBUG) || (defined('DbDebug') && DbDebug)) && defined('DB_DEBUG_SCRIPT') && ($_db || $Db)) {
		$ret .= '<a href="javascript:void(0)" onclick="debug_popup(\''.DB_DEBUG_SCRIPT.'\',800,500)">db-debug ('.$count_queries.')</a>';
		$xdebug_sep = true;
	}
